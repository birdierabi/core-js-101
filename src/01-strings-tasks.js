...
function getRectangleString(width, height) {
  const rectTopLine = `┌${'─'.repeat(width - 2)}┐\n`;
  const rectBody = (`│${' '.repeat(width - 2)}│\n`).repeat(height - 2);
  const rectBottomLine = `└${'─'.repeat(width - 2)}┘\n`;
  return rectTopLine + rectBody + rectBottomLine;
}
...
function encodeToRot13(str) {
  ...
  return str.replace(regexp, (letter) => alphabet[alphabet.indexOf(letter) + 13]);
}
